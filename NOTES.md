...
| E07 / T03–T05 | Amendments and foreign forms cannot be treated as interchangeable with domestic originals. | The endpoint uses exact case-sensitive form matching after trimming, including forms beyond the example list. Summary counts keep exact forms; no eligible exact 10-K returns null rather than substituting 20-F. | [Listing units](src/server/filings/tests/service.test.ts) cover 10-K, 10-K/A and 20-F; [route tests](src/server/filings/tests/routes.integration.test.ts) verify exact filtering over archived data. [Summary tests](src/server/summary/tests/aggregate.test.ts) cover exact forms and missing annual dates; UI behavior remains T05. |
...
they do not establish production readiness. Remaining UI behavior is labeled as planned.
